...
<html lang="en">
    <head>
        ...
        <title>Edit Public Notice Page | Student Management System</title>
        ...
        <div class="container container-content">
            <div class="content">
                <div class="content-header">
                    <h4 class="header-title">Edit Public Notice</h4>
                </div>
                <% if (request.getAttribute("errorMessage") != null) { %>
                    <p class="alert alert-failed"><%= request.getAttribute("errorMessage") %></p>
                <% } %>
                <form action="EditPublicNoticeServlet" method="post" class="form" style="grid-template-columns: auto;">
                    <input type="hidden" name="id" value="<%= publicNotice.getId() %>">
                    <div class="form-group">
                        <label for="name">Name</label>
                        <input type="text" id="name" class="input" name="name" required placeholder="Enter public notice name..." value="<%= publicNotice.getName() %>">
                    </div>
                    <div class="form-group">
                        <label for="description">Description</label>
                        <textarea id="description" class="input" name="description" required placeholder="Enter public notice description..." rows="6"><%= publicNotice.getDescription() %></textarea>
                    </div>
                    <div class="button-group">
                        <button type="submit" class="button-primary" style="width: 100%; text-align: center;">Save Changes</button>
                        <a href="AllPublicNoticeServlet" class="button-secondary" style="width: 100%; text-align: center;">Cancel Edit</a>
                    </div>
                </form>
            </div>
        </div>
